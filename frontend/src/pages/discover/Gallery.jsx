      ...
      <div className="mx-auto w-full space-y-6 px-4 py-10 sm:px-6 lg:px-10">
        <div className={containerCard}>
          <div className="flex items-center justify-between">
            <div>
              <h1 className="text-2xl font-semibold text-slate-900 dark:text-white">Gallery</h1>
              <p className="text-sm text-slate-600 dark:text-slate-300">Browse images and highlights from our programs.</p>
            </div>
            <Link to="/about" className="text-sm text-sky-600">About us</Link>
          </div>
            ...
            {loading ? (
              <div className="grid gap-4 sm:grid-cols-2 md:grid-cols-3 lg:grid-cols-4">
                {Array.from({ length: limit }).map((_, i) => (
                  <div key={i} className="h-44 rounded-md bg-slate-100 animate-pulse" />
                ))}
              </div>
            ) : error ? (
