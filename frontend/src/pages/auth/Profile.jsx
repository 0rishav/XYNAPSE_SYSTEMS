...
function Profile() {
  ...
  const loadProfile = useCallback(async () => {
    try {
      ...
      setProfileStatus({ type: "success", message: data.message || "Profile loaded" });
    } catch (error) {
  ...
  const submitPasswordChange = async (event) => {
    ...
    try {
      ...
      setPasswordStatus({ type: "success", message: response.message || "Password updated" });
  ...
  const submitProfileUpdate = async (event) => {
    ...
    try {
      ...
      setProfileFormStatus({ type: "success", message: response.message || "Profile updated" });
    } catch (error) {
      ...
      <div className="grid gap-8 lg:grid-cols-3">
        <div className="space-y-6 lg:col-span-1">
          <div className="rounded-3xl border border-slate-200/80 bg-white/90 p-6 shadow-sm dark:border-slate-800/80 dark:bg-slate-950/50">
            <div className="flex items-center justify-between">
              <p className="text-sm font-semibold text-slate-500 dark:text-slate-400">Profile snapshot</p>
              {profileLoading && (
                <span className="text-xs font-medium text-slate-400 dark:text-slate-500">Loading...</span>
              )}
            </div>
              ...
              {profile ? (
                <>
                  ...
                  <div>
                    ...
                    <p className="text-base text-slate-700 dark:text-slate-200">{profile.email || "—"}</p>
                  </div>
                  <div>
                    <p className="text-xs uppercase tracking-wide text-slate-400">Mobile</p>
                    <p className="text-base text-slate-700 dark:text-slate-200">{profile.mobile || "—"}</p>
                  </div>
                  <div>
                    <p className="text-xs uppercase tracking-wide text-slate-400">Role</p>
                    <p className="text-base text-slate-700 dark:text-slate-200">{profile.role || "—"}</p>
                  </div>
                  <div>
          ...
          <div className="rounded-3xl border border-slate-200/80 bg-white/90 p-6 shadow-sm dark:border-slate-800/80 dark:bg-slate-950/50">
            <p className="text-sm font-semibold text-slate-500 dark:text-slate-400">Security status</p>
            <ul className="mt-4 space-y-3 text-sm text-slate-600 dark:text-slate-300">
              <li className="flex items-center justify-between rounded-2xl bg-slate-100/70 px-3 py-2 text-slate-700 dark:bg-slate-900/70 dark:text-slate-200">
                ...
                <span className={profile?.emailVerified ? "text-emerald-500" : "text-amber-500"}>
                  {profile?.emailVerified ? "Yes" : "Pending"}
                </span>
              </li>
              <li className="flex items-center justify-between rounded-2xl bg-slate-100/70 px-3 py-2 text-slate-700 dark:bg-slate-900/70 dark:text-slate-200">
                <span>Phone verified</span>
                <span className={profile?.phoneVerified ? "text-emerald-500" : "text-amber-500"}>
                  {profile?.phoneVerified ? "Yes" : "Pending"}
                </span>
              </li>
              <li className="flex items-center justify-between rounded-2xl bg-slate-100/70 px-3 py-2 text-slate-700 dark:bg-slate-900/70 dark:text-slate-200">
                <span>2FA enabled</span>
                <span className={profile?.isTwofaEnabled ? "text-emerald-500" : "text-slate-400"}>
        ...
        <div className="space-y-8 lg:col-span-2">
          <div className="rounded-3xl border border-slate-200/80 bg-white/95 p-6 shadow-sm dark:border-slate-800/80 dark:bg-slate-950/50">
            <div className="flex items-center justify-between">
              <div>
                <h2 className="text-lg font-semibold text-slate-900 dark:text-white">Update profile</h2>
            ...
            <form className="mt-6 space-y-5" onSubmit={submitProfileUpdate}>
              ...
              <div className="space-y-2">
                <label className="text-sm font-medium text-slate-700 dark:text-slate-200" htmlFor="mobile">
                  Mobile number
                </label>
                <input
                  id="mobile"
              ...
              <div className="space-y-2">
                <label className="text-sm font-medium text-slate-700 dark:text-slate-200" htmlFor="bio">
                  Bio
                </label>
                <textarea
                  id="bio"
          ...
          <div className="rounded-3xl border border-slate-200/80 bg-white/95 p-6 shadow-sm dark:border-slate-800/80 dark:bg-slate-950/50">
            ...
            <form className="mt-6 space-y-5" onSubmit={submitPasswordChange}>
              <div className="space-y-2">
                <label className="text-sm font-medium text-slate-700 dark:text-slate-200" htmlFor="oldPassword">
                  Current password
                </label>
                <input
                  id="oldPassword"
              ...
              <div className="space-y-2">
                <label className="text-sm font-medium text-slate-700 dark:text-slate-200" htmlFor="newPassword">
                  New password
                </label>
                <input
                  id="newPassword"
