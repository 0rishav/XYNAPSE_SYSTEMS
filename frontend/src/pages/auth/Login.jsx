...
function Login() {
  ...
  const handleSubmit = async (event) => {
    ...
    try {
      ...
      if (response?.tempToken) {
        setStatus({
          type: "success",
          message: "2FA required. Enter the OTP sent to your device to continue.",
        });
        // TODO: route to OTP verification flow when available
      } else {
        dispatch(
          setUser({
        ...
        setStatus({ type: "success", message: "Welcome back! Redirecting you now..." });
        setTimeout(() => navigate("/play"), 900);
      }
    } catch (error) {
      const message =
  ...
  return (
    <div className="relative z-10 mx-auto flex min-h-[calc(100vh-4rem)] w-full max-w-3xl flex-col items-center justify-center px-4 py-16">
      <div className="w-full rounded-3xl border border-slate-200/50 bg-white/80 p-8 shadow-2xl shadow-slate-900/5 backdrop-blur-xl dark:border-slate-700/50 dark:bg-slate-900/60" role="region" aria-labelledby="login-heading">
        <header className="space-y-2 text-center">
          <p id="login-heading" className="text-2xl font-semibold text-slate-900 dark:text-white">
            Welcome back
          </p>
        ...
        {status && (
          <div className={`mt-6 rounded-2xl border px-4 py-3 text-sm font-medium ${statusClasses}`} role="status">
            {status.message}
          </div>
        )}
        ...
        <form className="mt-8 space-y-5" onSubmit={handleSubmit}>
          ...
          {/* <div className="space-y-2">
            <label htmlFor="deviceName" className="text-sm font-medium text-slate-700 dark:text-slate-200">
        ...
        <p className="mt-6 text-center text-sm text-slate-500 dark:text-slate-400">
          ...
          <Link className="font-semibold text-sky-500 hover:text-sky-400" to="/signup">
            Create one
          </Link>
        </p>
      </div>
